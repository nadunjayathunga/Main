...
def settlement_days(invoices: list) -> int:
    ...
    for invoice in invoices:
        inv_value: float = fCollection.loc[(
            fCollection['invoice_number'] == invoice), 'invoice_amount'].iloc[0]
        ...
        if (inv_value - total_collection) == 0:
            ...
            inv_date: datetime = fCollection.loc[(
                fCollection['invoice_number'] == invoice), 'invoice_date'].iloc[0]
...
def cust_ageing(customer: str) -> pd.DataFrame:
    ledgers: list = dCustomers.loc[(
        dCustomers['Cus_Name'] == customer), 'Ledger_Code'].tolist()
    credit_days: int = int(
        dCustomers.loc[dCustomers['Cus_Name'].isin([customer]), 'Credit_Days'].iloc[0])
    invoices: list = list(set(fCollection.loc[fCollection['ledger_code'].isin(
        ledgers), 'invoice_number'].tolist()))
    ...
    for invoice in invoices:
        total_collection: float = cust_soa.loc[(cust_soa['invoice_number'] == invoice) & (
            cust_soa['voucher_date'] <= end_date), 'voucher_amount'].sum()
        inv_value: float = cust_soa.loc[(
            cust_soa['invoice_number'] == invoice), 'invoice_amount'].iloc[0]
        if (inv_value - total_collection) != 0:
            inv_value_list.append(inv_value - total_collection)
            days_passed: int = (end_date - cust_soa.loc[(cust_soa['invoice_number']
                                == invoice), 'invoice_date'].iloc[0] - timedelta(days=credit_days)).days
...
def customer_ratios(customers: list, fInvoices: pd.DataFrame, end_date: datetime, fCollection: pd.DataFrame,
    ...
    for customer in customers:
        customer_since: datetime = fInvoices.loc[(fInvoices['Cus_Name'] == customer), 'Invoice_Date'].min() if not pd.isna(
        ...
        cust_invoices: list = fInvoices.loc[(
            fInvoices['Cus_Name'] == customer), 'Invoice_Number'].to_list()
        ...
        credit_days: int = dCustomer.loc[dCustomers['Cus_Name'].isin(
            [customer]), 'Credit_Days'].iloc[0]
        date_established: datetime = dCustomer.loc[dCustomer['Cus_Name'].isin(
            [customer]), 'Date_Established'].iloc[0]
        ...
        last_sales_person: str = fInvoices.loc[(
            fInvoices['Invoice_Date'] <= end_date), 'Employee_Code'].tail(1).iloc[0]

        stats: dict = {'customer_since': "Not Applicable" if customer_since == "Not Applicable" else customer_since.strftime('%d-%m-%Y'), 
                       'total_revenue': round(total_revenue), 'credit_score': 0,
...
# customer_list = ['Qatar National Convention Centre','QUEUE BUSINESS','Professional Services W.L.L','The Market Hall L.L.C']
# customer_list = ['The Market Hall L.L.C']
customer_list = dCustomers['Cus_Name'].tolist()

customer_ratios(customers=customer_list, fInvoices=fInvoices,
                end_date=end_date, fCollection=fCollection, dCustomer=dCustomers)
