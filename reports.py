...
df_jobs = pd.read_excel(io=PATH, sheet_name='dJobs', usecols=['Job_Number', 'Customer_Code'])
...
def job_revenue(row) -> float:
    ...
    filt = (df_data['Job_Code'] == job_id) & (df_data['Ledger_Code'].isin(
        [4010201001, 4010201002, 4010201003, 4010201004]))
...
def profitability_overall() -> pd.DataFrame:
    ...
    jobs_list_filt = (df_data['Voucher_Date'] >= START_DATE) & (df_data['Voucher_Date'] <= END_DATE) & (df_data['Type'] == 'SI')
    ...
    overall_report.to_csv('overallDetails.csv')
...
report = profitability_overall()

report.to_csv('overall.csv')
